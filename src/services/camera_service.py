...
class VideoWorker(QThread):
    ...
    def __init__(self, camera_config, camera_id):
        ...
        self.paused = False
        self.counting = 0
        self.counting_direction = camera_config['counting_direction'] #Untuk menentukan arah hitung
        self.last_positions = {} #Untuk menyimpan posisi terakhir
    ...
    def check_movement_direction(self, track_id, y_pos):
        if track_id not in self.last_positions:
            self.last_positions[track_id] = y_pos
            return None
    ...
    def detect_and_track(self, frame, border_points=None, area_pred_points=None):
        ...
        for track in tracks:
            ...
            if border_points and not self.is_bbox_inside_polygon([x1, y1, x2, y2], border_points):
                continue
            #--------------body------------------
            # if class_name == 'kepala':
            #     deteksi_frame['kepala'] = True
            # elif class_name == 'body':
            #     deteksi_frame['body'] = True
            #--------------body------------------
        ...
        for track in tracks:
            ...
            if area_pred_points:
                ...
                if is_crossing_pred and track_id not in self.counted_tracks:
                    current_y = (y1 + y2) // 2
                    movement_direction = self.check_movement_direction(track_id, current_y)

                    if movement_direction == self.counting_direction:
                        if class_name in ['azko', 'kawan_lama@ungu', 'kawan_lama@abu', 'informa', 
                                    'driver_informa', 'distribution_center', 'service_center', 
                                    'cipta_selera', 'elite', 'kawan_lama@driver']:
                            self.counting += 1
